# Additional file 4 for BMC Systems Biology manuscript:
# Identification of internal standard addition error in 1H-NMR time-course data
# Stanislav Sokolenko, Marc G Aucoin
#
# This file contains the script used to analyze gam model performance.
# Simulation results are stored in an SQLite database.

# Note, this simulation took 12-18 hours on a virtual machine
...
mdl.range <- list('gam-3', 'gam-4', 'gam-5', 'gam-6')
...
gen_gam <- function(k) {
  ...
  f_smooth <- function(time, concentration) {
    ...
    model <- gam(concentration ~ s(time, bs='cr', k=k), data=d)
